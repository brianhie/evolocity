...
def parse_args():
    ...
    parser.add_argument('--namespace', type=str, default='pgk',
...
def setup(args):
    ...
    #plt.xlim([ 360, 370 ])
    #plt.savefig('figures/pgk_seq_len.png', dpi=300)
    #plt.close()
    #exit()
...
def evo_pgk(args, model, seqs, vocabulary):
    ######################################
    ...
    cache_prefix = 'target/ev_cache/pgk_knn50'
    try:
    ...
    plt.savefig('figures/scvelo__pgk_taxonomy_velogrid.png', dpi=500)
    plt.close()
    ...
    plt.tight_layout(pad=1.1)
    plt.subplots_adjust(right=0.85)
    plt.savefig('figures/scvelo__pgk_taxonomy_velostream.png', dpi=500)
    plt.close()
    ...
    if args.evolocity:
        if args.checkpoint is None and not args.train:
            raise ValueError('Model must be trained or loaded '
                             'from checkpoint.')
        evo_pgk(args, model, seqs, vocabulary)
