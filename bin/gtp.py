...
def parse_args():
    import argparse
    parser = argparse.ArgumentParser(description='Obg-hflX GTPase sequence analysis')
    parser.add_argument('model_name', type=str,
                        help='Type of language model (e.g., hmm, lstm)')
    parser.add_argument('--namespace', type=str, default='gtp',
...
def parse_meta(record, taxonomy):
    ...
    if 'hflx' in name.lower():
        gene_group = 'hflx'
    elif 'ola1' in name.lower():
        gene_group = 'ola1'
    elif 'obg' in name.lower():
        gene_group = 'obg'
    elif 'drg' in name.lower():
        gene_group = 'drg'
    elif 'ychf' in name.lower():
        gene_group = 'ychf'
    else:
        gene_group = 'other'
...
def setup(args):
    fnames = [ 'data/gtp/uniprot_gtp.fa' ]

    seqs = process(fnames)

    #seq_lens = [ len(seq) for seq in seqs ]
    #plt.figure()
    #plt.hist(seq_lens, bins=3000)
    #plt.xlim([ 360, 370 ])
    #plt.savefig('figures/gtp_seq_len.png', dpi=300)
    #plt.close()
    #exit()
...
def plot_umap(adata, namespace='gtp'):
    sc.pl.umap(adata, color='tax_group', edges=True,
               save='_{}_taxonomy.png'.format(namespace))
    sc.pl.umap(adata, color='gene_group', edges=True,
               save='_{}_gene.png'.format(namespace))
...
def evo_gtpase(args, model, seqs, vocabulary):
    ######################################
    ...
    #new_seqs, gene_names = {}, { 'hflx', 'ola1', 'obg', 'ychf' }
    #for seq in seqs:
    #    for meta in seqs[seq]:
    #        if meta['gene_symbol'].lower() in gene_names:
    #            new_seqs[seq] = seqs[seq]
    #            break
    ...
    cache_prefix = 'target/ev_cache/gtp_knn30'
    try:
    ...
    plt.savefig('figures/scvelo__gtp_taxonomy_velogrid.png', dpi=500)
    plt.close()
    ...
    if args.evolocity:
        if args.checkpoint is None and not args.train:
            raise ValueError('Model must be trained or loaded '
                             'from checkpoint.')
        evo_gtpase(args, model, seqs, vocabulary)
